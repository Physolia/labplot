...
void addUniqueLine_double_vector_last_point_vector(double* p, double& minY, double& maxY, double* lastPoint, int& pixelDiff, QVector<QLineF>& lines, bool& prevPixelDiffZero);
void addUniqueLine_double_vector_last_point_vector_lines_vector(double* p, double& minY, double& maxY, double* lastPoint, int& pixelDiff, double *lines, bool& prevPixelDiffZero, int &numberLines);
void addUniqueLine_double_vector_last_point_vector_lines_vector_add4(double* p, double& minY, double& maxY, double* lastPoint, int& pixelDiff, double* lines, bool& prevPixelDiffZero, int& numberLines);
void addUniqueLine_double_vector_last_point_vector_lines_vector_add4_dont_copy_last_point(double* p, double& minY, double& maxY, double** lastPoint, int& pixelDiff, double* lines, bool& prevPixelDiffZero, int& numberLines);
...
void XYCurveTest::addUniqueLineTest01() {
	 ...
	 * - Chaching calculations. So (qRound64(x / minLogicalDiffX)) must not be done every time.
	 * - Try to get as much as possible calculations away. instead of abs() use != 0 which is secure for long int
	 */
...
//		x = 0;
//		PERFTRACE(QString(Q_FUNC_INFO) + "XYCurve::addUniqueLine use array access");
//		for (int i=0; i < count; i++) {
//			pixelDiff = llabs(qRound64(points[i].x() / minLogicalDiffX) - qRound64(x / minLogicalDiffX)) > 0; // only relevant if greater zero or not
...
//	}
...

	{
		...
		QVector<QLineF> lines;
		{
			...
			bool prevPixelDiffZero = false;
			x = 0;
			PERFTRACE(QString(Q_FUNC_INFO) + "011_XYCurve::addUniqueLine use @access try to simplify calculation, cache x/minLogicalDiffX");
			for (int i=0; i < count; i++) {
...
//		x = 0;
//		for (int i=0; i < count; i++) {
//			qint64 res = llabs(qRound64(points.at(i).x() / minLogicalDiffX) - qRound64(x / minLogicalDiffX));
//			pixelDiff_.append(res);
//			pixelDiff = res > 0; // only relevant if greater zero or not
//			XYCurvePrivate::addUniqueLine(points.at(i), minY, maxY, lastPoint, pixelDiff, lines, prevPixelDiffZero);
//			if (pixelDiff > 0) // set x to next pixel
//				x += minLogicalDiffX;
//		}
...
//}

	{
		...
		QVector<QLineF> lines;
		{
			...
			for (int i=0; i < count; i++) {
				pixelDiff = llabs(qRound64(pointerArray[i].x() / minLogicalDiffX) - qRound64(x / minLogicalDiffX)) > 0; // only relevant if greater zero or not
...

	{
		...
		QVector<QLineF> lines;
		{
			...
			for (int i=0; i < count; i++) {
				pixelDiff = llabs(qRound64(pointerArray[2*i] / minLogicalDiffX) - qRound64(x / minLogicalDiffX)) > 0; // only relevant if greater zero or not
...

	{
		...
		{
			bool prevPixelDiffZero = false;
			PERFTRACE(QString(Q_FUNC_INFO) + "04_XYCurve::addUniqueLine use raw double vector last point double vector");
			for (int i=0; i < count; i++) {
				pixelDiff = llabs(qRound64(pointerArray[2*i] / minLogicalDiffX) - qRound64(x / minLogicalDiffX)) > 0; // only relevant if greater zero or not
				addUniqueLine_double_vector_last_point_vector(&pointerArray[2*i], minY, maxY, lastPoint, pixelDiff, lines, prevPixelDiffZero);
				if (pixelDiff > 0) // set x to next pixel
					x += minLogicalDiffX;
			}
		}
...

	{
		...
		QVector<QLineF> lines2;
		{
			...
			PERFTRACE(QString(Q_FUNC_INFO) + "05_XYCurve::addUniqueLine use raw double vector last_point_double lines_double_vector");
			for (int i=0; i < count; i++) {
				pixelDiff = llabs(qRound64(pointerArray[2*i] / minLogicalDiffX) - qRound64(x / minLogicalDiffX)) > 0; // only relevant if greater zero or not
				addUniqueLine_double_vector_last_point_vector_lines_vector(&pointerArray[2*i], minY, maxY, lastPoint, pixelDiff, lines, prevPixelDiffZero, number_lines);
				if (pixelDiff > 0) // set x to next pixel
					x += minLogicalDiffX;
			}
		...
		qDebug() << x;
		QFile f("05_addUniqueLine_use_raw_double_vector_last_point_double_vector_lines_double_vector.txt");
		f.open(QIODevice::OpenModeFlag::WriteOnly);
...

	{
		...
		QVector<QLineF> lines2;
		{
			...
			int number_lines = 0;
			double* lines = (double*)malloc(maximumLines * 4 * sizeof(double));
			PERFTRACE(QString(Q_FUNC_INFO) + "051_XYCurve::addUniqueLine use raw double vector cache x/minLogicalDiffX");
			for (int i=0; i < count; i++) {
		...
		qDebug() << x;
		QFile f("051_addUniqueLine_use_raw_double_vector_last_point_double_vector_lines_double_vector_cache_x_minLogicalDiffX.txt");
		f.open(QIODevice::OpenModeFlag::WriteOnly);
		for (auto& line: lines2) {
			QTextStream out(&f);
			out << lineToString(line) << "\n";
...

	{
		...
		QVector<QLineF> lines2;
		{
			...
			int number_lines = 0;
			double* lines = (double*)malloc(maximumLines * 4 * sizeof(double));
			PERFTRACE(QString(Q_FUNC_INFO) + "0511_XYCurve::addUniqueLine use raw double vector cache x/minLogicalDiffX remove abs");
			for (int i=0; i < count; i++) {
		...
		qDebug() << x;
		QFile f("0511_addUniqueLine_use_raw_double_vector_last_point_double_vector_lines_double_vector_cache_x_minLogicalDiffX.txt");
		f.open(QIODevice::OpenModeFlag::WriteOnly);
		for (auto& line: lines2) {
			QTextStream out(&f);
			out << lineToString(line) << "\n";
...
//		{
//			bool prevPixelDiffZero = false;
//			PERFTRACE(QString(Q_FUNC_INFO) + "052_XYCurve::addUniqueLine use raw double vector cache x/minLogicalDiffX without linecopy");
//			for (int i=0; i < count; i++) {
//				pixelDiff = llabs(qRound64(pointerArray[2*i] / minLogicalDiffX) - r) > 0; // only relevant if greater zero or not
//				addUniqueLine_double_vector_last_point_vector_lines_vector(&pointerArray[2*i], minY, maxY, lastPoint, pixelDiff, lines, prevPixelDiffZero, number_lines);
//				if (pixelDiff > 0) { // set x to next pixel
...
//		}
//		qDebug() << x;
//		QFile f("052_addUniqueLine_use_raw_double_vector_last_point_double_vector_lines_double_vector_cache_x_minLogicalDiffX_without_line_copy.txt");
//		f.open(QIODevice::OpenModeFlag::WriteOnly);
//		for (int i=0; i < number_lines; i++) {
//			QTextStream out(&f);
...
//	}

	{
		...
		{
			bool prevPixelDiffZero = false;
			PERFTRACE(QString(Q_FUNC_INFO) + "053_XYCurve::addUniqueLine use raw double vector cache x/minLogicalDiffX without linecopy number_lines add 4 directly");
			for (int i=0; i < count; i++) {
				pixelDiff = llabs(qRound64(pointerArray[2*i] / minLogicalDiffX) - r) > 0; // only relevant if greater zero or not
				addUniqueLine_double_vector_last_point_vector_lines_vector_add4(&pointerArray[2*i], minY, maxY, lastPoint, pixelDiff, lines, prevPixelDiffZero, number_lines);
				if (pixelDiff > 0) { // set x to next pixel
					x += minLogicalDiffX;
					r = qRound64(x / minLogicalDiffX);
				}
...
//		{
//			bool prevPixelDiffZero = false;
//			PERFTRACE(QString(Q_FUNC_INFO) + "06_XYCurve::addUniqueLine use raw double vector cache x/minLogicalDiffX without linecopy number_lines add 4 directly don't copy last point");
//			for (int i=0; i < count; i++) {
//				pixelDiff = llabs(qRound64(pointerArray[2*i] / minLogicalDiffX) - r) > 0; // only relevant if greater zero or not
//				addUniqueLine_double_vector_last_point_vector_lines_vector_add4_dont_copy_last_point(&pointerArray[2*i], minY, maxY, &lastPoint, pixelDiff, lines, prevPixelDiffZero, number_lines);
//				if (pixelDiff > 0) { // set x to next pixel
//					x += minLogicalDiffX;
//					r = qRound64(x / minLogicalDiffX);
//				}
...
void addUniqueLine02(QPointF p, double& minY, double& maxY, QPointF& lastPoint, int& pixelDiff, QVector<QLineF>& lines, bool& prevPixelDiffZero) {
	...
	if(pixelDiff == 0){
		...
		if (prevPixelDiffZero) {
			if (maxY != minY) {
				lines[i] = QLineF(lastPoint.x(), minY, lastPoint.x(), maxY);
				i++;
			}
